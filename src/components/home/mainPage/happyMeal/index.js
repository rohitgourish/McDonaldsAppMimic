...
const HappyMealsComponent = (props) => {
    ...
    const addItem = (data) => {
        ...
        if(array.includes(data.id)){
            ...
            toast.success(`${data.name} Item Added To Cart!`,{ position: toast.POSITION.TOP_RIGHT, autoClose: 2500});
        }
    }
    ...
    const renderAllItems = () => {    
        if (props.products.length !== 0) {
            return props.products.map(product => {
                if(product["tags"].includes("happyMeals")){
...
export default connect(mapStateToProps, {getData,addItemToCart})(HappyMealsComponent)
